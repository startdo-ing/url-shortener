...
import {
	...
	type DomainDnsVerificationConfig
} from "./domain.ts"
...
describe("createDomainRepository", () => {
	...
	it("sets exactly one domain as primary and clears the flag from others", async () => {
		...
		// domainB should not be primary
		const list = await repository.listDomains()
		const bEntry = list.find((d) => d.id === domainB.id)
		expect(bEntry?.isPrimary).toBe(false)

		// Switching primary to domainB clears it from domainA
		await repository.setPrimaryDomain(domainB.id)
		const updatedList = await repository.listDomains()
		expect(updatedList.find((d) => d.id === domainA.id)?.isPrimary).toBe(false)
		expect(updatedList.find((d) => d.id === domainB.id)?.isPrimary).toBe(true)
	})
})
